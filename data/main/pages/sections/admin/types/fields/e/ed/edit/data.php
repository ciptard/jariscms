...
row: 0
	...
	field: content
		<?php
			...
			JarisCMS\Security\ProtectPage(array("view_types_fields", "edit_types_fields"));

			if(isset($_REQUEST["btnSave"]) && !JarisCMS\Form\CheckFields("edit-type-fields"))
			{
				$fields = JarisCMS\Field\GetTypeData($_REQUEST["id"], $_REQUEST["type_name"]);
				...
				if(JarisCMS\Field\EditType($_REQUEST["id"], $fields, $_REQUEST["type_name"]))
				{
					JarisCMS\System\AddMessage(t("The content type field has been successfully modified."));
				}
				else
				{
			...
			$parameters["action"] = JarisCMS\URI\PrintURL("admin/types/fields/edit");
			$parameters["method"] = "post";

			$fields[] = array("type"=>"hidden", "name"=>"id", "value"=>$_REQUEST["id"]);
			$fields[] = array("type"=>"hidden", "name"=>"type_name", "value"=>$_REQUEST["type_name"]);
			$fields[] = array("type"=>"text", "value"=>$_REQUEST["variable_name"]?$_REQUEST["variable_name"]:$field_data["variable_name"], "name"=>"variable_name", "label"=>t("Variable name:"), "id"=>"variable_name", "required"=>true, "description"=>t("The name of the variable used for this field when generating the form code."));
			...
			$fields[] = array("type"=>"textarea", "value"=>$_REQUEST["description"]?$_REQUEST["description"]:$field_data["description"], "name"=>"description", "label"=>t("Description:"), "id"=>"description", "required"=>true, "description"=>t("A brief description of how the user should fill this field or it's purpose."));
			...
			$fields[] = array("type"=>"text", "value"=>$_REQUEST["width"]?$_REQUEST["width"]:$field_data["width"], "name"=>"width", "label"=>t("Image width:"), "id"=>"width", "description"=>t("Maximum width of the image in pixels in case this field is an image upload. 0 for unlimited."));
