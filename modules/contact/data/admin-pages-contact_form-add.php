...
row: 0
    ...
    field: content
        ...
        <?php
            ...
            if(isset($_REQUEST["btnSave"]) && !JarisCMS\Form\CheckFields("add-page-{$_REQUEST['type']}") && JarisCMS\Group\GetTypePermission($_REQUEST["type"], JarisCMS\Security\GetCurrentUserGroup(), JarisCMS\Security\GetCurrentUser()))
            {
                //Trim uri spaces
                ...
                if(JarisCMS\Page\Create($_REQUEST["uri"], $fields, $uri))
                {
                    ...
                    //Add default fields
                    $fields["name"] = "Name";
                    $fields["variable_name"] = str_replace("-", "_", JarisCMS\URI\FromText($fields["name"]));
                    $fields["type"] = "text";
                    $fields["required"] = true;
                    $fields["strip_html"] = true;
                    $fields["position"] = "0";

                    JarisCMS\Module\ContactForms\AddField($fields, $uri);
            ...
            $parameters["action"] = JarisCMS\URI\PrintURL(JarisCMS\Module\GetPageURI("admin/pages/contact-form/add", "contact"));
            ...
            $fieldset[] = array("name"=>t("Subject"), "fields"=>$fields_subject, "collapsible"=>true, "description"=>t("Optional list of selectable subjects by the visitor with alternate mail recipients."));

            $fields_recipient[] = array("type"=>"text", "name"=>"mail_recipient", "value"=>$_REQUEST["mail_recipient"], "label"=>t("Mail recipient"), "id"=>"title", "description"=>t("The default recipient to receive the email. This value gets overriden by the recipient entered on the list of subjects."), "required"=>true);

            $fields_recipient[] = array("type"=>"textarea", "name"=>"mail_carbon_copy", "value"=>$_REQUEST["mail_carbon_copy"], "label"=>t("Carbon copy recipients"), "id"=>"title", "description"=>t("A comma seperated list of emails to receive a copy. For example: email_1@domain.com, email_2@domain.com"));
